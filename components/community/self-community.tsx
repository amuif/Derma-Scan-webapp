...
export function SelfCommunity() {
  ...
  useEffect(() => {
    ...
    const mine =
      scans.filter((s) => {
        ...
        return (
          s?.userId === uid ||
          (s as any)?.authorId === uid ||
          (s as any)?.ownerId === uid ||
          (s as any)?.createdById === uid ||
          s?.user?.id === uid
        );
      }) ?? [];
    setSelfScans(mine);
      ...
      <div className="space-y-6">
        ...
        <Card className="rounded-2xl border-rose-500/20 bg-rose-500/5">
          <CardContent className="flex items-center gap-2 p-6 text-rose-600">
            ...
            <span>Failed to fetch your scans. Please try again.</span>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="space-y-6">
        ...
        <Card className="rounded-2xl">
          <CardContent className="p-6 text-sm text-muted-foreground">
            Please sign in to view your scans.
          </CardContent>
        </Card>
      </div>
    );
  ...
  /* ------------------------------- empty -------------------------------- */
  if (!selfScans || selfScans.length === 0) {
    return (
      <div className="space-y-6">
        <section className="relative overflow-hidden rounded-3xl border bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-6">
          <div className="flex items-center gap-3">
