...
export default function LandingPage() {
  ...
  useEffect(() => {
    const setFromHash = () => setActiveHash(window.location.hash.replace("#", ""));
    setFromHash();
...
>
  ...
  >
    <svg className="absolute inset-0 h-full w-full">
      <defs>
        <pattern id="hdr-grid" width="28" height="28" patternUnits="userSpaceOnUse">
          <path d="M 28 0 L 0 0 0 28" fill="none" stroke="currentColor" strokeWidth=".5" />
        </pattern>
      </defs>
      <rect width="100%" height="100%" fill="url(#hdr-grid)" className="text-foreground/10" />
    </svg>
  </div>
        ...
        {[
          ...
          <a
            ...
            className={[
              ...
              "hover:bg-muted/60",
              activeHash === id ? "text-primary" : "text-muted-foreground",
            ].join(" ")}
          >
            {label}
            {/* underline accent */}
      ...
      {[
        ...
        <a
          ...
          className={[
            ...
            activeHash === id ? "bg-primary/10 text-primary" : "hover:bg-muted/60",
          ].join(" ")}
        >
          {label}
        </a>
  ...
  <div className="container mx-auto grid lg:grid-cols-2 items-center gap-16 px-6">
    ...
    <div>
      ...
      <p className="text-lg text-muted-foreground mb-8 max-w-lg">
         DermaScan estimates your skin-cancer risk with a clear confidence score,
         advises whether to contact a doctor, and can analyze your text description too.
      </p>
    ...
    <div className="relative flex items-center justify-center">
      ...
      <div className="relative z-10 rounded-3xl border border-border bg-background/70 shadow-xl backdrop-blur-md px-8 py-6">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold">DermaScan AI Engine Active</span>
        </div>
      ...
      <div className="absolute -bottom-10 flex gap-3">
        {[
          ...
          ["Privacy-first", "M12 2l7 4v6c0 5-3.5 9-7 10-3.5-1-7-5-7-10V6l7-4z"],
          ["Doctor guidance", "M12 21C7 21 3 17 3 12S7 3 12 3s9 4 9 9-4 9-9 9z M12 7v5l3 3"],
        ].map(([label, d], i) => (
          <div
            key={label}
      ...
      {/* ============ Divider ============ */}
      <div aria-hidden className="my-10 h-px w-full bg-gradient-to-r from-transparent via-border to-transparent" />
              ...
              >
                <CardHeader className="flex flex-row items-center gap-3 p-6 pb-3">
                  <div className="rounded-xl bg-primary/10 p-3">
                    {React.createElement(f.icon, { className: "h-6 w-6 text-primary" })}
                  </div>
      ...
      <section id="how-it-works" className="bg-muted/30 py-24">
        <div className="container mx-auto px-6">
          ...
          <div className="grid gap-6 md:grid-cols-4">
            {[
              { icon: Upload, title: "Upload", desc: "Take or upload a clear photo of the skin area." },
              { icon: MessageSquareText, title: "Describe (Optional) ", desc: "Add what you’re noticing—color, change, size." },
              { icon: Cpu, title: "AI analysis", desc: "Receive a cancer-risk estimate with a confidence score." },
              { icon: CheckCircle2, title: "Next steps", desc: "Doctor advice, community share, or clinic referral." },
            ].map((s, i) => (
              <Card key={i} className="rounded-2xl">
                <CardContent className="p-6">
                  <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10">
                    {React.createElement(s.icon, { className: "h-6 w-6 text-primary" })}
                  </div>
          ...
          <div className="mb-8 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-muted-foreground">
                Share anonymized results to raise awareness and help others know what to look for.
              </p>
            </div>
            ...
            {[1, 2, 3].map((i) => (
              <Card key={i} className="rounded-2xl">
                ...
                <CardContent className="text-sm text-muted-foreground">
                  “AI flagged moderate risk at 82% confidence. Booking a clinic visit.”
                </CardContent>
              </Card>
            ))}
          </div>
      ...
      <section id="clinics" className="bg-muted/30 py-20">
        <div className="container mx-auto grid items-center gap-6 px-6 lg:grid-cols-2">
          <div>
            <h2 className="text-3xl font-semibold tracking-tight">Find or refer a clinic</h2>
            <p className="mt-3 text-muted-foreground">
              If your result suggests follow-up, get routed to nearby partners or submit a clinic referral.
            </p>
            <div className="mt-6 flex gap-3">
              ...
              <Button asChild variant="outline" className="rounded-xl">
                <Link href={token ? "/clinics/refer" : "/login"}>Refer a Clinic</Link>
              </Button>
            </div>
          </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Find trusted care close to you.</p>
                </div>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-3">
                {["City Derm Clinic", "Skin Health Center", "Medix Specialized", "SunCare Dermatology"].map((c) => (
                  <div key={c} className="rounded-lg border border-border/60 p-3 text-sm">
                    {c}
                  </div>
                ))}
              </div>
      ...
      <section className="py-20">
        <div className="container mx-auto px-6">
          <h2 className="mb-6 text-3xl font-semibold tracking-tight">Built for</h2>
          <Tabs defaultValue="patients" className="w-full">
            ...
            <TabsContent value="patients" className="mt-6">
              <Card className="rounded-2xl p-6">
                <p className="text-muted-foreground">
                  Simple upload, clear AI risk + confidence, and actionable next steps.
                </p>
              </Card>
            </TabsContent>
            <TabsContent value="clinics" className="mt-6">
              <Card className="rounded-2xl p-6">
                <p className="text-muted-foreground">
                  Referral routing and context from user submissions to support triage.
                </p>
              </Card>
            </TabsContent>
            <TabsContent value="awareness" className="mt-6">
              <Card className="rounded-2xl p-6">
                <p className="text-muted-foreground">
                  Opt-in community posts to educate others on early signs to notice.
                </p>
              </Card>
            </TabsContent>
          </Tabs>
      ...
      <section id="faq" className="py-20">
        <div className="container mx-auto px-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="rounded-2xl">
              <CardHeader>
                <CardTitle className="text-base">Is DermaScan a medical diagnosis?</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-muted-foreground">
                No. It’s an AI risk estimate with a confidence score. Always consult a clinician for diagnosis.
              </CardContent>
            </Card>

            <Card className="rounded-2xl">
              <CardHeader>
                <CardTitle className="text-base">What photos work best?</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-muted-foreground">
                Sharp, well-lit close-ups without glare or blur. Include a ruler or coin for scale if possible.
              </CardContent>
            </Card>

            <Card className="rounded-2xl">
              <CardHeader>
                <CardTitle className="text-base">Do you store my photos?</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-muted-foreground">
                You control sharing. By default, images are processed securely and aren’t published without consent.
              </CardContent>
            </Card>

            <Card className="rounded-2xl">
              <CardHeader>
                <CardTitle className="text-base">How do referrals work?</CardTitle>
              </CardHeader>
              <CardContent className="text-sm text-muted-foreground">
                If risk is elevated, we surface nearby clinics and let you submit a referral or contact them directly.
              </CardContent>
            </Card>
          </div>
        </div>
